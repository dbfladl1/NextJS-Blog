...
export const PostCard = ({ post, refreshHandler }: PostCardProps) => {
  ...
  return (
    <div className="relative inline-block paper-flutter bg-[#fbf7efda] sm:h-[300px] h-full shadow-[1px_1px_0px_#d1cfc9] border border-[#e0ded9] p-5 pt-10 sm:w-[calc(33.333%-1rem)] w-full sm:min-w-[200px] rotate-[0.7deg]">
      ...
      <p className="text-md text-gray-700 scroll-auto">{post.text}</p>
      <p className="text-right mt-5 text-sm  text-gray-400">
        from. {post.nickname} (
        {post.relation === "friend"
          ? "친구"
          : post.relation === "coworker" && "직장 동료"}
        )
      </p>
    </div>
  );
